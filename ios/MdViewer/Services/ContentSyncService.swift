...
/// Coordinates the manifest → diff → download flow. Stateless beyond the
/// ContentStore it writes to; safe to instantiate per-sync.
@Observable
final class ContentSyncService {
    ...
    private func performSync(manifestURLString: String) async throws {
        ...
        for (index, entry) in toDownload.enumerated() {
            progress = SyncProgress(phase: .downloadingFiles, current: index, total: toDownload.count)
    ...
    private func downloadEntry(_ entry: ContentEntry, baseURL: URL) async throws {
    ...
    /// Files that exist locally but no longer in the remote manifest are removed.
